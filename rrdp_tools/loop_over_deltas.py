...
def attempt_delta_download(url_template: str, base_path: Path, min_delta: int, max_delta: int) -> None:
    for delta_number in range(min_delta, max_delta):
        try:
            get_and_check(base_path / f"{delta_number}.xml", url_template.format(delta_number))
...
def main():
    ...
    parser.add_argument('start',
                        help='Minimum number to template',
                        type=int
                        )
    parser.add_argument('end',
                        help='Final number to template',
                        type=int
                        )
